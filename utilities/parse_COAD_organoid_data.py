...
def return_COAD_organoid_drug_response_IC50():
	...
	fi_directory = '/content/drive/MyDrive/4060HW/final/organoid_biomarker_detection/data/organoid_COAD/drug_response'
...
def parse_GPL16686():
	...
	fi_directory = '/content/drive/MyDrive/4060HW/final/organoid_biomarker_detection/data/organoid_COAD/expression/GPL16686'
...
def return_COAD_2015_cell_organoid_RMA_normalized_expression():
	...
	fi_directory = '/content/drive/MyDrive/4060HW/final/organoid_biomarker_detection/data/organoid_COAD/expression/GSE64392/GSE64392_series_matrix.txt'
	...
	if not 'geneID_expression_median.txt' in fiList:
		...
		sampleList = list(output.keys())
		sampleList.sort()
		print('\t'.join(['geneID', 'uniprotID']) + '\t' + '\t'.join(map(str, sampleList)), file=fo)
		for gene in geneList:
			uniprot = gene2uniprot[gene]
			tmp_output = [gene, uniprot]
			for sample in sampleList:
				tmp_output.append(output[sample][gene])
			print('\t'.join(map(str, tmp_output)), file=fo)
		fo.close()

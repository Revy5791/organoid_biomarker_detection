...
def reactome_genes(): # provide in a dictionary
    ...
    output_list = []
    f = open('/content/drive/MyDrive/4060HW/final/organoid_biomarker_detection/data/msigdb.v6.1.symbols.gmt.txt','r')
    for line in f:
...
def annotation():
    ...
    df = pd.read_csv('/content/drive/MyDrive/4060HW/final/organoid_biomarker_detection/data/2017_07_31_biomart_protein_coding_genes.txt', sep='\t')
...
def geneID2uniprot():
    ...
    df = pd.read_csv('/content/drive/MyDrive/4060HW/final/organoid_biomarker_detection/data/uniprot_homoSapiens_multipleGeneName_20180802.tab', sep='\t')
    for i in range(len(df)):
